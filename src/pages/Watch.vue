...
async function loadVideo() {    
  ...
  try {    
    ...
    console.error('APIの取得に失敗しました。URLから直接再生を行います。', error)
  } finally {    
    loading.value = false    
  }    
}    
      ...
      >    
        ...
        <div class="min-w-0">    
          ...
          <div class="overflow-hidden rounded-2xl bg-black">    
            <VideoPlayer    
              :key="`${selectedStream}-${video?.videoId || route.params.id}`"    
              :id="video?.videoId || (route.params.id as string)"    
          ...
          >    
            ...
            <div class="mt-3 whitespace-pre-wrap text-[15px] leading-7 text-zinc-800">    
              {{    
                shortDescription || '動画の説明を取得できませんでした。'    
              }}    
